...
function batchTimeouts(callbacks, delays) {
  ...
  let timeOutObjectArray = [];
  for(i in callbacks){

    // for each callback, match with its corrsponding delay at the same index
    let currentCallback = callbacks[i];
    let currentDelay= delays[i];

    // add each timeout return object to an array
    timeOutObjectArray.push(setTimeout(currentCallback, currentDelay));
  }
  return timeOutObjectArray;
}
